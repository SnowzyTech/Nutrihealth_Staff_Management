    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          <div className="h-10 w-10 rounded-lg bg-[#FE871F]/20 border border-[#FE871F]/30 flex items-center justify-center">
            <BookOpen className="h-5 w-5 text-[#FE871F]" />
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-[#43005F]">Staff Handbook</h1>
            <p className="text-gray-600 text-xs sm:text-sm">Nutrihealth Consult Employee Handbook</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-10 bg-gray-50 border-gray-200 text-[#43005F] placeholder:text-gray-500 text-sm"
              />
              {searchTerm && (
                <button
            ...
            <Button
              ...
              className={showBookmarksOnly
                ? 'bg-[#FE871F] hover:bg-[#E67E1B] text-white h-9 text-xs sm:text-sm'
                : 'bg-transparent border-gray-200 text-[#43005F] hover:bg-gray-100 h-9 text-xs sm:text-sm'}
            >
              <Bookmark className="h-3.5 w-3.5 mr-1.5" />
              Bookmarks
              {bookmarks.length > 0 && (
                <Badge className="ml-1.5 bg-[#FE871F]/20 text-[#FE871F] border-0 text-[10px] px-1.5">
                  {bookmarks.length}
                </Badge>
              )}
            </Button>
        ...
        <div className="space-y-3">
          ...
          {searchResults.length === 0 ? (
            ...
            searchResults.map((result) => (
              ...
              >
                <Card className="bg-white border-gray-200 hover:border-[#FE871F]/40 transition-colors cursor-pointer mb-2">
                  <CardContent className="p-3 sm:p-4">
                    <div className="flex items-start gap-3">
                      <BookOpen className="h-4 w-4 text-[#FE871F] mt-0.5 flex-shrink-0" />
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-[#43005F]">{result.sectionTitle}</p>
          ...
          {bookmarks.length === 0 ? (
            <Card className="bg-white border-gray-200">
              <CardContent className="py-12 text-center">
                ...
                <p className="text-gray-700 font-medium">No bookmarks yet</p>
                <p className="text-gray-500 text-sm mt-1">Open a chapter and bookmark sections you want to revisit</p>
              </CardContent>
            </Card>
          ) : (
            bookmarks.map((bm) => {
              ...
              return (
                ...
                >
                  <Card className="bg-white border-gray-200 hover:border-[#FE871F]/40 transition-colors cursor-pointer mb-2">
                    <CardContent className="p-3 sm:p-4">
                      <div className="flex items-start gap-3">
                        <Bookmark className="h-4 w-4 text-[#FE871F] mt-0.5 flex-shrink-0 fill-[#FE871F]" />
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-[#43005F]">{section.title}</p>
                          <p className="text-xs text-gray-600 mt-0.5">
      ...
      {!searchTerm.trim() && !showBookmarksOnly && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-primary uppercase tracking-wider">Table of Contents</h2>
          ...
          {HANDBOOK_CHAPTERS.map((chapter) => {
            ...
            return (
              <Card
                key={chapter.id}
                className="bg-white border-gray-200 shadow-lg overflow-hidden"
              >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-sm sm:text-base font-semibold text-primary/80 truncate">
                            {chapter.title}
                          </h3>
                        </div>
                        <p className="text-xs text-slate-700 mt-0.5 truncate hidden sm:block">
                          {chapter.description}
                        </p>
                  ...
                  <div className="border-t border-gray-200 bg-white">
                    {chapter.sections.map((section, idx) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`flex items-center gap-3 px-4 sm:px-6 py-2.5 sm:py-3 hover:bg-slate-200 transition-colors cursor-pointer group ${
                              idx < chapter.sections.length - 1 ? 'border-b border-slate-700/50' : ''
                            }`}
                          >
                            <span className="text-[10px] sm:text-xs font-mono text-primary w-6 sm:w-8 text-right flex-shrink-0">
                              {section.number}
                            </span>
                            <span className="text-xs sm:text-sm text-primary group-hover:text-slate-700 transition-colors flex-1 min-w-0 truncate">
                              {section.title}
                            </span>
                            {isBookmarked && (
